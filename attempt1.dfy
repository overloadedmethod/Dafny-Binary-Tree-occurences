...
lemma L1(nt: Tree, n': int, key:int, nt1: Tree, nt2: Tree, sum1: nat, sum2: nat)
	requires sum2 == NumbersInTree(nt2)[key] // pre0
	requires sum1 == NumbersInTree(nt1)[key] // pre1
	requires  nt == Node(n',nt1,nt2) // pre2
	ensures (if n' == key then 1 else 0) + sum1 + sum2 == NumbersInTree(nt)[key]
{
	calc {
	...
	== // pre0 and pre1
		(if n' == key then 1 else 0) + sum1 + sum2;
	}
}
